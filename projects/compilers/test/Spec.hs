...
cinstructionsAndOutputs = [(dIn <> cIn <> jIn, "111" <> cOut <> dOut <> jOut) | (dIn, dOut) <- dests, (cIn, cOut) <- comps, (jIn, jOut) <- jumps] where
  dests = 
    [("", "000")
    ,("M=", "001")
    ,("D=", "010")
...
testAssembler = testGroup "Assembler tests"
  [testCase command $ assemble command @?= Just output | (command, output) <- instructionsAndOutputs
  ]
